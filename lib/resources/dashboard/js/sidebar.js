(function () {
	var key = $.cookie('bosBackendSshKey');
	_bosBackend.ajax.headers = {
...
$(document).ready(function () {
	...
	function loadResourceTypes() {
		bosBackend('__resources').get('types', function (res, err) {
	...
	function loadResources() {
		bosBackend('__resources').get(function (res, err) {
	...
	function deleteResource(resource) {
		ui.confirm('Delete ' + resource.id + '?', 'This cannot be undone!')
			...
			.show(function (ok) {
				if (ok) {
					bosBackend('__resources').del(resource.id, function (res, err) {
						if (err) {
	...
	function renameResource(resource) {
		pathDialog('Rename', 'Rename ' + resource.id, '/' + resource.id, function (newPath) {
			...
			resource.id = newPath;
			bosBackend('__resources').put(oldPath, resource, function (res, err) {
				if (err) {
