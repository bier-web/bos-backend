...
(function () {
	...
	vm.view.scrollToRow = function (row, fn) {
		...
		setTimeout(function () {
			...
			if (rowIndex === -1) {
				bosBackend(resource).get('index-of', rowId, function (res) {
					var index = res.index,
	...
	function createRow(data) {
		...
		rowVm._deleteRow = function () {
			if (rowVm.id()) {
				...
				bosBackend(resource).del(rowVm.id(), { $skipEvents: true }, function (res, err) {
		...
		rowVm._saveProp = function (prop, value, options) {
			...
			if (lastValue !== value) {
				...
				if (rowVm.id()) {
					...
					bosBackend(resource).put(rowVm.id(), body, function (res, err) {
						if (err) {
	...
	function postRow(data, fn) {
		...
		bosBackend(resource).post(data, function (res, err) {
	...
	function loadProperties(fn) {
		bosBackend('__resources').get(resource, function (res, err) {
	...
	function loadCount(fn) {
		bosBackend(resource).get('count', function (res) {
			vm.count(res.count);
			if (typeof fn === 'function') {
				fn();
			}
	...
	function loadPage(page, fn) {
		...
		bosBackend(resource).get({ $skip: begin * PAGE_SIZE, $limit: PAGE_SIZE * 3, $skipEvents: true }, function (res, err) {
			if (err) return;
			vm.currentPage(page);
	...
	loadProperties(function (props) {
		...
		if (props) {
			...
			bosBackend.on(resource + ':changed', function () {
				loadPage();
				loadCount();
			});
		} else {
