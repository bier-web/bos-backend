(function () {
	...
	function create() {
		...
		function fetchProperties() {
			bosBackend('__resources').get(Context.resourceId, function (res, err) {
		...
		bosBackend(Context.resourceId).get({ $limit: 1 }, function (res) {
			if (res.length) {
				vm.otherItems(true);
			} else {
				vm.otherItems(false);
			...
			ko.computed(function () {
				...
				propertiesArray.forEach(function (p) {
					...
					if (p.name !== p.id) {
						var rename = {};
						rename[p.id] = p.name;
						bosBackend(Context.resourceId).exec('rename', { properties: rename });
						p.id = p.name;
					}
				...
				.subscribe(function (propertiesJson) {
					bosBackend('__resources').put(Context.resourceId, { properties: propertiesJson }, function (res, err) {
						if (err)
							return ui
