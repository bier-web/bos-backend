...
module.exports.loadConfig = function (basepath, server, fn) {
	...
	async.waterfall(
		[async.apply(loadResourceDir, basepath), async.apply(loadResources, getTypes, basepath, server), async.apply(addInternalResources, server, basepath)],
		function (err, result) {
...
function addInternalResources(server, basepath, resources, fn) {
	...
	publicFolderPromise.then(function (publicFolder) {
		...
		if (typeof server.options === 'undefined') {
			debug('server.options is undefined. Will add (show) bosBackend.js and dashboard in internals array');
			internals.push(new ClientLib('bos-backend.js', { config: { resources: resources }, server: server }));
			internals.push(new Dashboard('dashboard', { server: server }));
		} else {
			if (typeof server.options.hide_bosBackendJs !== 'undefined' && server.options.hide_bosBackendJs === true) {
				//hide bosBackendJs.js and dashboard
				debug('Will not add (show) bos-backend.js and dashboard in internals array');
				...
				//show everything
				debug('Will add (show) bosBackend.js and dashboard in internals array');
				internals.push(new ClientLib('bos-backend.js', { config: { resources: resources }, server: server }));
				internals.push(new Dashboard('dashboard', { server: server }));
			}
		}
