...
exports.restart = function (mongod, env, port, fn) {
	...
	try {
		...
		pid = JSON.parse(fs.readFileSync('./.bosBackend/pids/mongod'));
	 ...
	 */
	var options = ['--dbpath', './data', '--pidfilepath', './.bosBackend/pids/mongod', '--port', port, '-f', fs.existsSync('/dev/null') ? '/dev/null' : 'NUL'];
	if (env === 'development') {
	...
	function kill(e) {
		...
		fs.writeFileSync('./.bosBackend/pids/mongod', '');
		proc.kill();
		process.exit(0);
	}
	...
	proc.on('error', function (err) {
		...
		// report error to startup function in bin/bosBackend
		fn(err);
	});
