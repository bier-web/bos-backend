	...
	fs.readdir(__dirname + '/resources', function (err, dir) {
		...
		if (fs.existsSync(packageJsonPath)) {
			...
			if (packageJson && packageJson.dependencies) {
				var dependencies =
					packageJson.bosBackendInclude && packageJson.bosBackendInclude.length
						? packageJson.bosBackendInclude.reduce(function (prev, curr) {
								prev[curr] = packageJson.dependencies[curr];
								return prev;
						  }, {})
						: packageJson.dependencies;
				var bosBackendIgnore = packageJson.bosBackendIgnore || [];
				debug('Loading these dependencies from package.json', dependencies);
				remaining = 0;
				for (var dependency in dependencies) {
					if (bosBackendIgnore.indexOf(dependency) === -1) loadCustomResources(dependency);
				}
			}
		} else {
