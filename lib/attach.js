...
/**
 * Attach bos router, sessions, db and functions into an existing http server instance.
...
function attach(httpServer, options) {
	...
	server.options = options = _.extend(
		{
			db: { port: 27017, host: '127.0.0.1', name: 'bosBackend' }
		},
		options
	);
		...
		setupReqRes(server.options, req, res, function (err, next) {
			...
			server.sessions.createSession(authToken || req.cookies.get('sid'), function (err, session) {
				if (err) {
					...
					var root = req.headers['bos-ssh-key'] || req.cookies.get('bosBackendSshKey');
